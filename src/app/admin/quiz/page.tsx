    ...
    <div>
      ...
      <div className="card mb-5 space-y-3 p-4">
        ...
        <select value={form.difficulty} onChange={(e) => setForm({ ...form, difficulty: Number(e.target.value) })} className="rounded-xl border border-white/10 bg-magic-900 px-3 py-2">
          <option value={1}>Facile (2 pts)</option>
          <option value={2}>Moyenne (4 pts)</option>
          <option value={3}>Difficile (6 pts)</option>
          <option value={4}>Expert (8 pts)</option>
          <option value={5}>Impossible (10 pts)</option>
        </select>
            ...
            <div className="flex items-start justify-between gap-2">
              <div className="flex-1">
                ...
                <div className="mt-0.5 text-[11px] text-white/40">
                  {['', '🟢 Facile', '🔵 Moyenne', '🟣 Difficile', '🟠 Expert', '🔴 Impossible'][q.difficulty] ?? `Niveau ${q.difficulty}`} · {q.difficulty * 2} pts
                </div>
              </div>
